...
def process_payment(ch, method, properties, body):
    ...
    if payment_success:
        ...
        print(f"[{data['student_name']}] Payment: Payment applied for Order {order_id}, events sent.")
    else:
        channel.basic_publish(exchange=EXCHANGE_NAME, routing_key="payment-denied", body=json.dumps(data))
        print(f"[{data['student_name']}] Payment: Payment denied for Order {order_id}, notification sent.")
    connection.close()
